    ...
    def request(self, method, end_point, key_check, data=None):
        try:
            ...
            if "failed to parse" in response.text:
                ...
                print_colored_log("Authentication renewed!")
                time.sleep(2)
        
        except requests.exceptions.ConnectionError:
            print_colored_log(f"ConnectionError {end_point}. Sleeping for 5s...")
            time.sleep(5)
        except urllib3.exceptions.NewConnectionError:
            print_colored_log(f"NewConnectionError {end_point}. Sleeping for 5s...")
            time.sleep(5)
        except requests.exceptions.Timeout:
            print_colored_log(f"Timeout Error {end_point}. Sleeping for 5s...")
            time.sleep(5)
        ...
        try:
            ...
            if charges > 0:
                for _ in range(charges):
                    ...
                    print_colored_log(f" {account_name} anti-detect: jeda {t}...")
                    time.sleep(t)
            else:
                print_colored_log(f"{account_name}: No charge available. Sleeping for 10 minutes...")
                time.sleep(600)
        except requests.exceptions.ConnectionError:
...
def mine_claimer(NotPxClient: NotPx, account_name: str):
    ...
    print_colored_log("Auto claiming started.")
    while True:
        ...
        for remaining in range(3600, 0, -1):
            ...
            print(f"{account_name}: Sleeping for {timer}...", end="\r")
            time.sleep(1)
        print(f"{account_name}: Sleeping for 1 hour complete!")
